import Image from 'next/image'
import React, { useState, useEffect } from 'react'
import axios from 'axios'
import _ from 'lodash'

const PartnersSection = () => {
    const [data, setData] = useState([])
    useEffect(() => {
        axios({
            method: "get",
            url: "http://localhost:1337/api/datacards/32?populate[0]=listitems&[populate][1]=listitems.datacard,listitems.datacarditems,listitems.subcards",
            responseType: "json",
            headers: { "Authorization": "Bearer " }
        })
            .then((res) => {
                if (!_.isEmpty(res.data))
                    setData(res.data)
            })
            .catch((err) => {
                console.log("bad request", err.message)
            })
    }, [])

    const heading = _.get(data, "data.attributes.title")
    const cardData = _.get(data, "data.attributes.listitems.data", [])
    console.log(cardData)


    return (
        <section className='partners-section'>

            <h2 className='heading02 text-center'>{heading}</h2>
            <div className='container-s'>
                

                    {
                        cardData.map((imagesbox) => {
                            let imagedata = _.get(imagesbox, "attributes.datacarditems", [])
                            console.log(imagedata)
                            return (
                                <div className='partners-grid'>
                                    {
                                        imagedata.map((images) => {
                                            return (
                                                <div className='grid-box'>
                                                <Image src={images.item} layout="fill" className='resp-img' alt="" />
                                                </div>
                                            )
                                        })
                                    }
                                    {/* <Image src={images.path} layout="fill" className='resp-img' alt="" /> */}
                                </div>
                            )
                        })
                    }
                    {/* {
                    box.map(images =>(
                        <div className='grid-box'><Image src={images.path} layout="fill" className='resp-img' alt="" /></div>
                        ))
                } */}
                    {/* <div className='grid-box'><Image src="/img/Aicpa.jpg" layout="fill" className='resp-img' /></div>
                <div className='grid-box'><Image src="/img/BDO_logo.jpg" layout="fill" className='resp-img' /></div> */}
                {/* </div> */}
            </div>
        </section>
    )
}

export default PartnersSection  