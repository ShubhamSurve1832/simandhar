import Image from 'next/image'
import { useEffect, useState } from 'react'
import axios from 'axios'
import _ from "lodash"
import React from 'react'

const BannerSection = ({ bannerData, showData,dataCard_id}) => {

  const [data, setData] = useState([])
  useEffect(() => {
    axios({
      method: "get",
      url: "http://localhost:1337/api/datacards/"+dataCard_id+"?populate[0]=listitems&[populate][1]=listitems.datacard,listitems.datacarditems,listitems.subcards",
      responseType: 'json',
      headers: { "Authorization": "Bearer " },
    })
      .then((res) => {
        if (!_.isEmpty(res.data))
          setData(res.data)
      }
      )
      .catch((err) =>
        console.log("here is error", err.message)
      )
  }, [])

  let heading = _.get(data, "data.attributes.title")
  let description = _.get(data, "data.attributes.desc")
  let cardData = _.get(data, "data.attributes.listitems.data", [])
  // console.log("here is title", cardData)

  // export default function bannerSection() {
  return (
    <>
      {
        cardData.map((data => {
          let sectiondata = _.get(data, "attributes.datacarditems")
          let img_url = _.get(data, "attributes.image_url")
          return (
            <section className="program-banner ">
              <div className="banenr-left resp-img-box">
                <Image src={img_url} alt="" layout="fill" className='resp-img' />
              </div>
              <div className="progam-info">
                <h2 className="heading02">{heading}</h2>
                <ul>
                  {
                    sectiondata.map((lists => {
                      let list = _.get(lists, "item")
                      // console.log("here is title2", list)
                      return <li>{list}</li>
                    }))
                  }
                </ul>
                {
                  showData ? (<h3 className='title01'>{description} </h3>) : (" ")
                }
              </div>
            </section>
          )
        }))
      }


      {/* {

        cardData.map((data => {
          let sectiondata = _.get(data, "attributes.datacarditems")
          let img_url = _.get(data, "attributes.image_url")
          console.log("here is title1", sectiondata, img_url)

          return (
            <>
              {
                sectiondata.map((lists => {
                  let list = _.get(lists, "item")
                  console.log("here is title2", list)

                }))
              }
            </>
          )
        }))

      } */}
    </>
  )
}

export default BannerSection
